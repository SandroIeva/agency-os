...
- **Artboard text on a ring is drawn three times.** `canvasArcLayout` decides where every letter sits; the editor and the board thumbnail share `CanvasArcText` and the PNG export walks the same layout onto a canvas. The box IS the circle — its width is the diameter and `canvasTextH` returns that width, so a text item is square while `arc.on` is set. A fourth drawer that reimplements the maths puts the export somewhere the screen never was, which is the same trap the line breaker (`canvasTextLines`) exists to close.
