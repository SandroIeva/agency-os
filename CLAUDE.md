...
| **Link folder** | `addFolder` in `LinksTab`, plus the `seed_link_folders` trigger on `organizations` | App.jsx / Postgres |
...
- **Brand:** `brand_profile`, `brand_shares`, `brand_canvases` (ONE jsonb doc
  ...
  the browser)
- **Misc:** `notifications`, `reminders`, `calendar_events`, `notes`, `push_subscriptions`, `short_links`, `os_visuals`, `workspace_links` (the Links tab of the file manager: url, title, note and a `folder_id` into **`link_folders`** (its own table so a folder can be empty and still exist; every workspace is seeded with Skills / Tools / Inspirations / Mockups by an `after insert` trigger on `organizations`). `category` is the free-text grouping folders replaced, kept for one release and no longer written. Org- and project-scoped like everything else. Saving a link also stores what the page says about itself: `favicon` is a **data: URL** fetched server-side and kept inline, so drawing the list makes no request to the linked site; `image_url` is the og:image, kept remote on purpose and to be rendered through `api/img-proxy`; `site` is og:site_name. Rows saved before this fill themselves in on load, 8 at a time)
