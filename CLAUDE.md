...
- **Instagram, direkt über Meta:** `instagram_connections` (service key only, RLS on with zero policies; PK `(org_id, ig_user_id)`, several accounts per workspace). Beside Zernio, never instead of it: `INSTAGRAM_DIRECT_ORGS` names the workspaces allowed to use the direct path, and an empty list means nobody, which is what makes it safe to ship before Meta has reviewed anything. Instagram has NO refresh token: a live 60-day token is traded for a fresh one, so it must happen before the old one lapses (10 days of margin, and never under 24 hours of age, which Instagram refuses); a lapsed one can only be authorised again.
...
**Meta App Review**: what i7OS submits, in what order, and the traps (screen recordings in English at 1080p, a 1024x1024 icon we do not yet have, and one successful API call per permission within 30 days of submitting) is written down in `docs/meta-app-review.md`. Live mode is NOT Advanced Access: a permission reading "Ready for testing" serves only accounts with a role on the Meta app.

`notion` (**edge**: read-only, one connection per WORKSPACE in `notion_connections` (service key only, RLS on with zero policies). `?check=1` needs no secret; `?mode=install&state=<token>` uses the same one-time `create_messenger_link_token` (kind `notion`, and the callback insists on it); `/notion/callback` is rewritten in vercel.json. POST modes: `status` / `disconnect` / `search` / `page`. `page` turns a Notion page into HTML (`server/notion.js`, tested by `npm run test:notion`) and the browser imports it exactly like a Google Doc in `importFromNotion` (DocsTab): images copied into our storage (Notion's own file links expire within the hour, hence the `/api/img-proxy` fallback in `rehostDocImages`), HTML parsed into BlockNote blocks. Tokens are refreshed on a 401, since Notion does not document their lifetime. Needs `NOTION_CLIENT_ID`, `NOTION_CLIENT_SECRET`).

`instagram` (**edge**: Instagram straight through Meta, next to Zernio. `?check=1` reports the wiring and the commit and needs no secret; `?mode=install&state=<token>` uses the same one-time `create_messenger_link_token`; `/instagram/callback`, `/instagram/deauthorize`, `/instagram/delete` and `/instagram/delete-status` are rewritten in vercel.json. POST modes: `status` / `disconnect` / `publish` / `publish-finish` / `overview` / `insights` / `limit`. Needs `INSTAGRAM_APP_ID`, `INSTAGRAM_APP_SECRET` - the **Instagram** app id and secret from the Instagram product page, NOT the Meta app's and not the Threads ones - plus `INSTAGRAM_DIRECT_ORGS`).

`media-host` (**edge**: wo eine große Datei wartet, während Meta sie sich abholt. Instagram und Threads nehmen keine Bytes, nur eine URL, und unser Supabase-Speicher steht auf dem kostenlosen Plan mit **50 MB je Objekt**, was für Video nichts ist. Alles über 45 MB geht deshalb stückweise auf einen eigenen Webspace, den diese Funktion mit kurzlebigen HMAC-Tickets absichert; die BYTES gehen nie durch die Funktion, der Browser lädt direkt dorthin. `?check=1` braucht kein Geheimnis und meldet auch, was das Skript drüben sagt. Alles dazu, samt der beiden Dateien zum Hochladen, steht in `docs/media-relay.md`. Braucht `MEDIA_HOST_URL`, `MEDIA_HOST_SECRET`; ohne beides antwortet es 503 und der Composer bleibt bei 50 MB).
...
- **Artboard text can be styled per SELECTION, and `runs` is how.** A text element carries one set of properties; `runs` is a list of `{ from, to, … }` over character offsets that overrides them, and `CANVAS_RUN_KEYS` names everything one may carry. Offsets are into the RAW text (the case setting preserves length), and they are shifted on every edit by `canvasShiftRuns` or a colour creeps across the words beside it. Three rules learned the hard way: a run's START and END answer differently when text is inserted exactly at them; a second property applied to part of a range must MERGE rather than replace; and neighbours that end up identical must be coalesced or the list grows on every change of mind. `scripts/test-canvas-text-runs.mjs` covers all three against the shipped code.
- **The line breaker measures per CHARACTER now**, because a size or a font on part of a text changes where a line breaks and how tall it is. `canvasTextLayout` is the one that knows; it returns each line's text, its offsets in the source, and its own height, and `canvasTextLines` is a thin wrapper over it. It is a FAITHFUL port of the old word-by-word breaker, right down to a run of spaces being a token that can trigger a break and a break trimming trailing whitespace while a paragraph end does not. `scripts/test-canvas-wrap.mjs` compares it against the old algorithm on plain text, which is the only thing standing between a change here and every existing artboard re-flowing.
- **A masked item is drawn by TWO renderers, and both have to clip.** `CanvasThumb` (the artboard cards, the board list, every preview) and the editor draw the same board from the same items. The card filtered masks out of what it drew and then never clipped anything with them, so a photograph that was a circle in the editor was a full-bleed rectangle in every preview. `canvasMaskClip(it, items)` is at module scope for exactly that reason and both call it.
- **A mask on the artboard is a PAIR plus a group.** The clipping shape carries `isMask`, everything it clips carries `maskId` pointing at that shape's id, and all of them share one `groupId` so dragging cannot pull them apart. `maskClip` turns the shape into a CSS clip-path, which means a rectangle, an ellipse or a polygon and NOT a free path. Figma says the same thing the other way round: a mask is a child that clips its siblings above it, up to the next mask, and children arrive bottom first. Two traps: `walkNode` turns a masking node away at the door, so the container has to walk it with the flag removed; and anything that hands out new ids (`placeFigmaItems`) must carry `maskId` through the mapping, or a second paste is clipped by the first paste's mask.
- **Artboard text on a ring is drawn three times.** `canvasArcLayout` decides where every letter sits, and hands back a 2x3 **matrix** per glyph rather than a position and an angle: the editor and the thumbnail share `CanvasArcText` (CSS `matrix()`), the PNG export feeds the same numbers to `ctx.transform`. A rotation is not enough — in `mode: "cyl"` a letter is foreshortened as it turns away and mirrored once it faces the back. The box width is always the ring's diameter; the HEIGHT comes from `canvasTextH`, which asks the layout (square when flat, a shallow band when leaning). A fourth drawer that reimplements the maths puts the export somewhere the screen never was, which is the same trap the line breaker (`canvasTextLines`) exists to close.
...
- **A Figma fill with several subpaths is ONE artboard path, never several.** A ring, a letter's counter and a boolean result come over as one `d` holding several subpaths, and which side of each is inside is the FILL RULE's answer. A fill rule cannot see across two elements, so one item per subpath means the outer one is simply painted over the others: an imported copyright mark arrived as a solid white dot. The first subpath lives in `nodes` (what the node editor edits), the rest in `subs`, appended to the same `d` by `pathFullD`. Anything that draws or measures a path has to use it, `fitItems` has to scale `subs` with `nodes`, and the remap that folds ox/oy in when a board changes format has to map them too.
- **When an import "looks wrong", read the board back out of `brand_canvases` and render it.** The doc is `{ boards: [{ name, w, h, items }] }`, and 40 lines of Node that walk the items with the same `pathD` the app uses produce an SVG that reproduces the screen exactly. That turned three rounds of guessing about coordinate spaces into one measurement, and it also says whether the item on screen is even the one the current code produced.
- **Instagram publishing is three calls and one of them is slow.** Container, poll, publish. A picture is FINISHED in a second or two; a reel is transcoded and routinely takes a minute, which is longer than an Edge function may sit there. So `publish` waits as long as it safely can and answers **202 `{ status: "processing", containerId }`**; the browser then polls **`publish-finish`**, which is the same check and the same publish with NO container built. A poll that could build one would post twice.
- **Instagram fetches the media itself and will not take bytes.** Pinterest accepts base64; Instagram accepts only a url it can reach. Files from our private buckets therefore go over as a **signed url** minted server-side, which is why the composer's Meta path uploads to `brand-assets` and hands the function a `{ bucket, path }` rather than reusing Zernio's presigned upload.
...
- **BlockNote writes an inline style two different ways, and only one is the inline one.** A highlighted RUN renders as `<span data-style-type="backgroundColor" data-value="green">`; `[data-background-color=green]` is what a whole coloured BLOCK gets. Its own stylesheet covers both, so an override that matches only the second silently does nothing to selected text while looking correct. Read the attribute off the real DOM (a throwaway page mounting `BlockNoteView` is enough) before writing a selector for it.
...
- **A throwaway test page under `vercel dev` has nowhere to live.** `cleanUrls` strips `.html` and the SPA catch-all then swallows the path, so the page renders as the app; rename it `.htm` and Vite no longer injects the React preamble, so a JSX probe dies with "can't detect preamble". Write such a probe as plain HTML with no build step, or serve it from `npm run dev` (5173), which has neither rewrite.
...
- Trigger logic can be tested safely against production: wrap test inserts in a plpgsql `BEGIN … EXCEPTION` block that ends with `raise exception 'undo'` — the savepoint rolls everything back.
  **EVERY write must be inside the block that raises.** A test with several inner `BEGIN … EXCEPTION` blocks rolls back only the one that raised; writes in the earlier blocks are committed. This overwrote a real workspace's brand claim in production on 2026-09-08, and it was only caught by reading the row back afterwards. Two rules that would have caught it: one raising block around ALL of it, and always SELECT the row after the test to prove it is unchanged.
  Note also that `set_config(..., false)` inside a block that rolls back is rolled back too. Capture results into a plpgsql variable and `set_config` them in the OUTER block, after the inner one has unwound. To make `auth.uid()` return a user inside such a test, `perform set_config('request.jwt.claims', json_build_object('sub', <uuid>)::text, true)`.
